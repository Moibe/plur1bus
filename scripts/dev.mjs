 ...
 *   API_DIR               (default: ../plur1bus-api)
 *   API_HOST              (default: 127.0.0.1)
 *   API_PORT              (default: 8004)
...
const API_DIR = resolve(ROOT, process.env.API_DIR ?? "../plur1bus-api");
const API_HOST = process.env.API_HOST ?? "127.0.0.1";
const API_PORT = Number(process.env.API_PORT ?? 8004);
...
function shutdown(code = 0) {
	...
	for (const p of [webProc, apiProc]) {
		if (p && p.exitCode === null) {
			try {
				// En Windows, kill() solo termina el cmd.exe de shell:true y deja vivo
				// a vite (nieto); taskkill /T se lleva el árbol completo.
